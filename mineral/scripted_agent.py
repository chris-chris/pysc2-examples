  ...
  def step(self, obs):
    ...
    if _MOVE_SCREEN in obs.observation["available_actions"] and r == 0:
      ...
      for p in zip(neutral_x, neutral_y):
        ...
        if(other_dest):
          dist = numpy.linalg.norm(numpy.array(other_dest) - numpy.array(p))
          if(dist<5):
      ...
      solve_tsp = False
      if(my_dest):
        dist = numpy.linalg.norm(numpy.array(player) - numpy.array(my_dest))
        if(dist < 2):
          solve_tsp = True
      ...
      if(solve_tsp):
        ...
        left_d = numpy.linalg.norm(numpy.array(player) - numpy.array(left))
        right_d = numpy.linalg.norm(numpy.array(player) - numpy.array(right))
        if(right_d > left_d):
          closest = left
        else:
          closest = right
      ...
      return actions.FunctionCall(_SELECT_CONTROL_GROUP, [[_CONTROL_GROUP_RECALL], [int(self.group_id)]])
    else:
      return actions.FunctionCall(_NO_OP, [])
